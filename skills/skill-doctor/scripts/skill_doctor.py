...
IGNORED_TREE_PARTS = {"__pycache__", ".pytest_cache", ".mypy_cache", "obj", "bin"}
...
def validate_cursor_cloud_discovery(repository_root: Path, skills_root: Path, skill_names: set[str], issues: list[Issue]) -> None:
    ...
    if discovery_path.is_symlink() or discovery_path.is_junction():
        add_issue(
            ...
            discovery_path,
            ".cursor/skills must be a real directory; links are not portable across checkouts.",
        )
        return
    ...
    if not discovery_path.is_dir():
        add_issue(
            ...
            "invalid-cursor-skills-discovery",
            discovery_path,
            ".cursor/skills must be a real directory.",
        )
        return
...
def tree_digest(root: Path) -> str:
    ...
    for path in sorted(root.rglob("*")):
        ...
        if root.name == "software-engineering-graph" and relative.parts[0] in {"policies", "state"}:
            continue
        if not path.is_file() or path.suffix in IGNORED_SUFFIXES or any(part in IGNORED_TREE_PARTS for part in relative.parts):
            continue
