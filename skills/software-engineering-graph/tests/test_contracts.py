...
class ContractTests(GraphCase):
    ...
    def test_missing_installed_policy_is_created_and_repository_policy_is_ignored(self):
        ...
        self.assertEqual(preview["code"], "EXECUTION_PLAN_PREVIEW")
        initialized = self.graphctl(
            "--ack-degraded-permissions", "--ack-degraded-durability", "init",
            "--run-id", "PROFILE-RUN", "--task-brief", str(task_path), "--op-id", "profile-init",
        )
        self.graphctl(
            "record", "plan-approval", "--run-id", "PROFILE-RUN",
    ...
    def test_generated_policy_excludes_existing_artifact_and_sensitive_roots(self):
        ...
        for root in ("credentials.json", "notes.key", "secrets-backup/"):
            with self.subTest(root=root):
                ...
                invalid["implementation_roots"].append(root)
                installed.write_text(json.dumps(invalid), encoding="utf-8")
                with self.assertRaisesRegex(ContractError, "SENSITIVE_PATH"):
                    load_policy(self.repo)
                invalid = copy.deepcopy(policy)
                invalid["role_capabilities"]["code_reviewer"].append({
                    "effect": "filesystem_read", "action": "read", "target_ref": "repo:" + root,
                })
                installed.write_text(json.dumps(invalid), encoding="utf-8")
                with self.assertRaisesRegex(ContractError, "SENSITIVE_PATH"):
                    load_policy(self.repo)
    ...
    def test_generated_policy_skips_top_level_junction(self):
        ...
        policy, _ = load_policy(self.repo)
        self.assertNotIn("linked-project/", policy["implementation_roots"])

    def test_policy_and_state_follow_each_install_location(self):
        source = Path(__file__).resolve().parents[1]
        environment = dict(os.environ, PYTHONDONTWRITEBYTECODE="1")
        for host in (".agents", ".claude", ".cursor", ".codex"):
            with self.subTest(host=host):
                installed = self.root / host / "skills" / "software-engineering-graph"
                shutil.copytree(source / "graph_engine", installed / "graph_engine")
                shutil.copytree(source / "scripts", installed / "scripts")
                completed = subprocess.run(
                    [sys.executable, str(installed / "scripts" / "graphctl.py"),
                     "--repo", str(self.repo), "policy"],
