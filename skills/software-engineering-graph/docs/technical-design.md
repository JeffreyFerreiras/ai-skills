...
its Sol/Luna recommendations; revision 5 raises GPT-6 Luna helper effort to max. `hosts.py` keeps the
...
New recommendations: Codex uses GPT-6 Luna max helpers and Astra medium core/high review, with
...
defaults, bytes, digests and approvals. Codex revisions 3 and 4 also retain their prior recommendations
and options. The engine accepts each known revision explicitly; unknown or malformed revisions fail
closed. New Codex revision 5 plans require this engine or newer. Catalog
...
It uses `git worktree add` to create a standard Git worktree, not a Codex-managed worktree.
