...
- The ai-skills repository is authoritative for maintained skill code. Installed profile code remains untouched unless
  separately approved work explicitly changes it. Normal graph runs may create or update per-repository policy under
  the installed skill's `policies/` directory and ledger state under its `state/` directory.
...
- Do not track cache, bytecode, virtual environment, database, run-state, inbox, secret, environment,
  coverage, build, or temporary artifacts in the source repository. When this directory is the
  installed skill, its `policies/` and `state/` are ignored runtime directories; never stage or sync them.
...
- Do not mutate a profile, consumer repository, or remote system without separately approved scope,
  except the installed skill's policy and ledger runtime writes required by an authorized graph run.
