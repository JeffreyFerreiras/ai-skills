...
Fail: blocks because the profile policy was initially absent, uses repository-local policy as
authority, treats policy creation as mutation authority, or silently accepts an invalid profile policy.
...
Expected: read instructions/status/registrations/base, create one standard Git worktree and branch
with `git worktree add` rather than Codex-managed `create_worktree`,
