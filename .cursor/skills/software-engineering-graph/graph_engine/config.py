...
def profile_policy_path(repo: Path, skill_root: Path) -> Path:
    return skill_root / "policies" / ("repo-" + sha256_bytes(_repository_identity(repo))[:24] + ".json")


def _default_profile_policy(repo: Path) -> Dict[str, Any]:
    artifact_root = "engineering-graph-artifacts/"
    implementation_roots = []
    for entry in sorted(repo.iterdir(), key=lambda item: item.name.lower()):
        if (entry.name.startswith(".") or entry.name == "AGENTS.md"
                or os.path.normcase(entry.name) == os.path.normcase(artifact_root.rstrip("/"))
                or entry.is_symlink()
        ...
        if len(implementation_roots) == MAX_AUTOGENERATED_ROOTS:
            break
    read_roots = [artifact_root] + implementation_roots
    ...
    for role in ENGINE_ROLE_CAPABILITIES:
        ...
        if role == "senior_engineer":
            role_capabilities.extend(
                ...
                for root in implementation_roots + [artifact_root]
            )
        elif role == "tech_lead":
            role_capabilities.append({
                "effect": "filesystem_write", "action": "edit", "target_ref": "repo:" + artifact_root,
            })
    ...
    return {
        "schema_version": 2, "repository_id": repository_id,
        "compatible_engine": {"min": "2.6.0", "max_exclusive": "3.0.0"},
        "artifact_roots": {"repo": [artifact_root], "profile": ["references/"]},
...
def _ensure_profile_policy(repo: Path, skill_root: Path) -> Path:
    path = profile_policy_path(repo, skill_root)
    ensure_safe_components(skill_root)
    path.parent.mkdir(parents=True, exist_ok=True)
    ensure_safe_components(path.parent)
    if not os.path.lexists(path):
        data = canonical_bytes(_default_profile_policy(repo))
        try:
...
def load_policy(repo: Path, skill_root: Optional[Path] = None) -> Tuple[Dict[str, Any], Snapshot]:
    root = repo.absolute()
    profile_root = (skill_root or SKILL_ROOT).resolve(strict=True)
    snapshot = safe_json_snapshot(_ensure_profile_policy(root, profile_root), [profile_root], 256 * 1024)
