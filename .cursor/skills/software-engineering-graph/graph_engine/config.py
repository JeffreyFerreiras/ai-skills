...
from .state import StateError, resolve_state_root


SKILL_ROOT = Path(__file__).resolve().parents[1]
MAX_AUTOGENERATED_ROOTS = 128


def _repository_identity(repo: Path) -> bytes:
    path = str(repo.resolve(strict=True))
    return (path.casefold() if os.name == "nt" else path).encode("utf-8")
...
def _validate_capability_shape(capability: Any, role: str, command_ids: Set[str]) -> Tuple[str, str, str]:
    ...
    if effect.startswith("filesystem"):
        ...
        if not target.startswith(("repo:", "runtime:", "profile:software-engineering-graph/")):
...
def _repository_target(repo: Path, target: str, denied_patterns: Any) -> str:
    ...
    for pattern in denied_patterns:
        ...
        if (fnmatch.fnmatchcase(normalized, lowered)
                or fnmatch.fnmatchcase("/" + normalized, lowered)
                or path.match(lowered)):
...
def _runtime_target(target: str, roots: Any) -> str:
    if not target.startswith("runtime:"):
        raise ContractError("capability.target_ref", "RUNTIME_REF_REQUIRED")
    relative = lexical_relative(target[len("runtime:"):], "capability.target_ref")
    if any(character in relative for character in "*?[]{}"):
        raise ContractError("capability.target_ref", "WILDCARD_FORBIDDEN")
    if not any(_under_configured_root("runtime:" + relative, [root]) for root in roots):
        raise ContractError("capability.target_ref", "OUTSIDE_ALLOWED_ROOT")
...
def _under_configured_root(target: str, roots: Any) -> bool:
    if not target.startswith(("repo:", "runtime:")):
        return False
...
def role_capability_allowed(
    ...
    if effect.startswith("filesystem"):
        if policy["schema_version"] == 3 and target.startswith("runtime:"):
            return _under_configured_root(target, policy["artifact_roots"]["runtime"])
        if not target.startswith("repo:"):
            return False
        if effect == "filesystem_write":
            if role == "senior_engineer":
                return _under_configured_root(
                    target, policy["implementation_roots"] +
                    (policy["artifact_roots"]["repo"] if policy["schema_version"] == 2 else []),
                )
            return policy["schema_version"] == 2 and role == "tech_lead" and _under_configured_root(
...
def profile_policy_path(repo: Path, runtime_root: Path) -> Path:
    """Policy path in the selected runtime home (legacy function name retained)."""
    return runtime_root / "policies" / ("repo-" + sha256_bytes(_repository_identity(repo))[:24] + ".json")


def _default_profile_policy(repo: Path) -> Dict[str, Any]:
    repository_id = "profile-" + sha256_bytes(_repository_identity(repo))[:24]
    artifact_root = "artifacts/" + repository_id + "/"
    implementation_roots = []
    for entry in sorted(repo.iterdir(), key=lambda item: item.name.lower()):
        if (entry.name.startswith(".") or entry.name == "AGENTS.md"
                or os.path.normcase(entry.name) == os.path.normcase("engineering-graph-artifacts")
                or entry.is_symlink()
                or (os.name == "nt" and bool(getattr(entry.lstat(), "st_file_attributes", 0) & stat.FILE_ATTRIBUTE_REPARSE_POINT))):
            continue
        relative = entry.name + ("/" if entry.is_dir() else "")
        try:
            lexical_relative(relative, "implementation_roots")
            _repository_target(repo, "repo:" + relative, ENGINE_DENIALS)
        except ContractError:
            continue
        if entry.is_dir() or entry.is_file():
            implementation_roots.append(relative)
        if len(implementation_roots) == MAX_AUTOGENERATED_ROOTS:
            break
    read_roots = implementation_roots
    check_id = "git-diff-check"
    capabilities = {}
    for role in ENGINE_ROLE_CAPABILITIES:
        role_capabilities = [
            {"effect": "filesystem_read", "action": "read", "target_ref": "repo:" + root}
            for root in read_roots
        ]
        role_capabilities.append({
            "effect": "filesystem_read", "action": "read", "target_ref": "runtime:" + artifact_root,
        })
        if role == "senior_engineer":
            role_capabilities.extend(
                {"effect": "filesystem_write", "action": "edit", "target_ref": "repo:" + root}
                for root in implementation_roots
            )
        if role in {"senior_engineer", "tech_lead"}:
            role_capabilities.append({
                "effect": "filesystem_write", "action": "edit", "target_ref": "runtime:" + artifact_root,
            })
        if role in {"senior_engineer", "code_reviewer", "test_engineer"}:
            role_capabilities.append({"effect": "command", "action": "run", "target_ref": check_id})
        capabilities[role] = role_capabilities
    templates = {}
    for key, (role, stages, kind, artifact_required, decisions, retry_ceiling) in ENGINE_NODE_SPECS.items():
        output = {"artifact_kind": kind, "schema_version": 1, "artifact_required": artifact_required}
        if decisions:
            output["decision_values"] = list(decisions)
        if key == "senior_engineer":
            output.update({
                "artifact_required_for_decisions": ["IMPLEMENTED"],
                "evidence_required_for_decisions": ["REDESIGN_REQUIRED"],
            })
        if key in ENGINE_RESEARCH_NODES:
            output.update({"evidence_required": True, "decision_forbidden": True, "findings_forbidden": True})
        templates[key] = {
            "role": role, "stages": sorted(stages), "mandatory": True,
            "max_retries": retry_ceiling, "output_contract": output,
        }
    return {
        "schema_version": 3, "repository_id": repository_id,
        "compatible_engine": {"min": "2.7.0", "max_exclusive": "3.0.0"},
        "artifact_roots": {"repo": [], "profile": ["references/"], "runtime": [artifact_root]},
        "implementation_roots": implementation_roots,
        "artifact_kinds": {
            kind: {"extensions": sorted(ENGINE_ARTIFACT_EXTENSIONS[kind]), "max_bytes": ceiling}
            for kind, ceiling in ENGINE_ARTIFACT_MAX.items()
        },
        "impact_tags": sorted(RISK_TAGS),
        "routes": {key: dict(route) for key, route in ENGINE_ROUTES.items()},
        "node_templates": templates,
        "specialists": {
            tag: {"node_key": node, "role": role, "stages": ["design", "delivery"], "mandatory": True}
            for tag, (node, role) in ENGINE_SPECIALISTS.items()
        },
        "limits": {
            "design_revisions": 2, "delivery_repairs": 3,
            "inspection": {"file_reads": 12, "discovery_commands": 8},
            "manifest_bytes": 256 * 1024, "artifact_bytes": 4 * 1024 * 1024,
        },
        "required_checks": {
            "diff-check": {
                "command_id": check_id, "mandatory": True,
                "argv": ["git", "diff", "--check"], "timeout_seconds": 60,
            },
        },
        "role_capabilities": capabilities,
        "denied_patterns": sorted(ENGINE_DENIALS),
    }


def _ensure_profile_policy(repo: Path, runtime_root: Path, seed: Optional[bytes] = None) -> Path:
    path = profile_policy_path(repo, runtime_root)
    ensure_safe_components(runtime_root)
    runtime_root.mkdir(mode=0o700, parents=True, exist_ok=True)
    path.parent.mkdir(mode=0o700, exist_ok=True)
    ensure_safe_components(path.parent)
    if os.name != "nt":
        os.chmod(runtime_root, 0o700)
        os.chmod(path.parent, 0o700)
    if not os.path.lexists(path):
        data = seed if seed is not None else canonical_bytes(_default_profile_policy(repo))
        try:
...
def load_policy(
    repo: Path, skill_root: Optional[Path] = None, runtime_root: Optional[Path] = None,
) -> Tuple[Dict[str, Any], Snapshot]:
    root = repo.absolute()
    installed_root = (skill_root or SKILL_ROOT).resolve(strict=True)
    home = resolve_state_root(runtime_root, skill_root=installed_root)
    policy_path = profile_policy_path(root, home)
    legacy_path = profile_policy_path(root, installed_root)
    if (not os.path.lexists(policy_path) and (home == installed_root / "state" or (home / "graph-runs").exists())
            and legacy_path.exists()):
        snapshot = safe_json_snapshot(legacy_path, [installed_root], 256 * 1024)
    else:
        try:
            default_home = resolve_state_root(skill_root=installed_root)
        except StateError:
            if runtime_root is None:
                raise
            default_home = None
        seed = None
        if default_home is not None and home != default_home and not os.path.lexists(policy_path):
            default_path = profile_policy_path(root, default_home)
            if default_path.exists():
                seed = safe_json_snapshot(default_path, [default_home], 256 * 1024).data
        snapshot = safe_json_snapshot(_ensure_profile_policy(root, home, seed), [home], 256 * 1024)
    ...
    root_kinds = ("repo", "profile", "runtime") if policy_version == 3 else ("repo", "profile")
    require_keys(roots, set(root_kinds), set(root_kinds), "artifact_roots")
    if policy_version == 3 and roots["repo"] != []:
        raise ContractError("artifact_roots.repo", "RUNTIME_ARTIFACTS_REQUIRED")
    if policy_version == 3 and roots["runtime"] != ["artifacts/" + value["repository_id"] + "/"]:
        raise ContractError("artifact_roots.runtime", "RUNTIME_ROOT_FORBIDDEN")
    for root_kind in root_kinds:
        configured = roots[root_kind]
        if (not isinstance(configured, list) or (not configured and (root_kind != "repo" or policy_version < 3))
        ...
        for path in configured:
            ...
            if root_kind == "runtime" and not normalized.startswith("artifacts/" + value["repository_id"] + "/"):
                raise ContractError("artifact_roots.runtime", "RUNTIME_ROOT_FORBIDDEN")
    ...
    for role, configured in capabilities.items():
        ...
        for item in configured:
            ...
            if policy_version >= 2 and item["effect"].startswith("filesystem"):
                configured_denials = value["denied_patterns"]
                if item["target_ref"].startswith("runtime:") and policy_version == 3:
                    normalized_target = _runtime_target(item["target_ref"], roots["runtime"])
                else:
